...
| 고객 온라인 예약 | `/book.html?s=<코드>`: 휴대폰 인증(OTP)만으로 가입, 빈 시간 조회→담당 디자이너 지정(필수, 자동 배정 없음)→예약→내 예약 취소. 매장별 링크는 설정 화면에서 발급·on/off·정책(자동확정/최소리드타임/최대예약일) 관리. Solapi 등 실연동 전까지는 인증번호 `000000`이 항상 통과(운영 환경에서는 자동 비활성화) |
